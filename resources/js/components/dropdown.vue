<template>
  <div class="relative">
    ...
    <div v-if="isOpen" class="absolute right-0 mt-1 py-2 w-48 bg-gray-300 rounded-lg z-20">
      <a href="#" class="block px-4 py-2 text-gray-800 hover:bg-indigo-500 hover:text-white">Change Group</a>
      <a href="#" class="block px-4 py-2 text-gray-800 hover:bg-indigo-500 hover:text-white">Account settings</a>
      <form action="/logout" method="post">
          <input type="hidden" name="_token" :value="csrf" /> 
          <button class="text-left block px-4 py-2 text-gray-800 h-full w-full outline-none border-none hover:bg-indigo-500 hover:text-white">Logout</button>
      </form>
    </div>
  </div>
</template>
